...
**The response variable is the Happiness_score.**\

What other factors do you think might impact the response variable?\

**Factors that might impact the Happiness_score include Alcohol consumption and attitudes towards Lying.**\
...
**There are `#N/A` values in the Dominant_hand and Lying columns.**\
...
table(happiness$Education)
table(happiness$Dominant_hand)
...
**Alcohol consumption and attitudes towards Lying probably have a significant impact on Happiness_score.**
...
**There is a min of -45 and a max of 75**
...
**The distribution of the happiness scores in the alcohol_use data appears to be approximately normal with a slight right skew.**

Create a side-by-side boxplot and summary statistics table for each level of alcohol use:

```{r}
boxplot(Happiness_score ~ Alcohol, data = alcohol_use,
        main = "Happiness Scores by Alcohol Use",
        xlab = "Alcohol Use", ylab = "Happiness Score",
        col = rainbow(length(unique(alcohol_use$Alcohol))))
# Summary statistics
favstats(Happiness_score ~ Alcohol, data = alcohol_use)

```

How different do the happiness scores appear to be between these two groups?\

**They don't appear to be very different.**\
...
ggplot(lying, aes(x = Lying, y = Happiness_score, fill = Lying)) +
  geom_boxplot() +
  labs(title = "Happiness Scores by Attitude about Lying",
       x = "Attitude about Lying",
       y = "Happiness Score") +
  theme_minimal() +
...
favstats(Happiness_score ~ Lying, data = lying)
...
```{r}
education_clean <- happiness %>%
  filter(Happiness_score <= 15,
         Happiness_score >= 0,
         Education != "nunya business") %>%
  select(Education, Happiness_score)

ggplot(education_clean, aes(x = Education, y = Happiness_score, fill = Education)) +
  geom_boxplot() +
  labs(title = "Happiness Scores by Education Level",
       x = "Education Level",
       y = "Happiness Score") +
  theme_minimal() +
  theme(legend.position = "none",
...
summary_stats_education <- favstats(Happiness_score ~ Education, data = education_clean)
...
**It appears that education does not make a significant difference on ones happiness score.**
